...
#ifdef __MORPHOS__
// no real use. just wanted to cut down code
...
static __inline__ amodes mode_from_str (const char *str)
{
    ...
    return (amodes)0;
#else
	ULONG a, *mode = (ULONG *)str;

	a	= *mode;

	if (a == MAKE_ID('D','r','e','g')) return Dreg;
	if (a == MAKE_ID('A','r','e','g')) return Areg;
	if (a == MAKE_ID('A','i','n','d')) return Aind;
	if (a == MAKE_ID('A','p','d','i')) return Apdi;
	if (a == MAKE_ID('A','i','p','i')) return Aipi;
	if (a == MAKE_ID('A','d','1','6')) return Ad16;
	if (a == MAKE_ID('A','d','8','r')) return Ad8r;
	if (a == MAKE_ID('a','b','s','w')) return absw;
	if (a == MAKE_ID('a','b','s','l')) return absl;
	if (a == MAKE_ID('P','C','1','6')) return PC16;
	if (a == MAKE_ID('P','C','8','r')) return PC8r;
	if (a == MAKE_ID('I','m','m','d')) return imm;
...
#else
		if (lookuptab[find].name[0] == 0) abort();
#endif
		}
	}
	else {
