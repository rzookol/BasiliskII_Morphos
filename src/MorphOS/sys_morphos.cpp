...
{
	// Amiga doesn't support MacOS partitioning, so this probably doesn't make much sense...
}
...
void *Sys_open(const char *name, bool read_only)
{
	...
	if (is_file)
	{
		...
		if (f)
		{
			...
			{
				success = ExamineFH64(f, &FIB, NULL);
			}
			else
			{
				success = ExamineFH(f, &FIB);
...
size_t Sys_read(void *arg, void *buffer, loff_t offset, size_t length)
{
	...
	if (fh->is_file) {
		...
		if (SysBase->LibNode.lib_Version >= 51)
		{
			if (Seek64(fh->f, offset + fh->start_byte, OFFSET_BEGINNING) == -1)
				return 0;
		}
		else
		{
